...
_TIME_SPEC = ("auto", "hours", "minutes", "seconds",
...
timedelta.max = timedelta(days=999_999_999, hours=23,
...
def _iso2t(s):
    ...
    if l != i:
        raise ValueError
    if tz_sign:
        td = timedelta(hours=tz_hour, minutes=tz_minute,
                       seconds=tz_sec, microseconds=tz_usec)
        if tz_sign == "-":
            td = -td
        tz = timezone(td)
    else:
